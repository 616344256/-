...
-(void)setPasswordTextField:(UITextField *)passwordTextField{
    ...
    UIImageView *imageViewPwd=[[UIImageView alloc]initWithFrame:CGRectMake(20, 0, 25, 15)];
    imageViewPwd.image=[UIImage imageNamed:@"人"];
    ...
    if (self.phoneTextField.text.length == 11) {
        if (self.codeTextField.text.length > 0) {
         if (self.passwordTextField.text.length >= 6) {
             ...
             [manager POST:@"http://suoadmin.slkfz.cn/api/api/register" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                 [[NSUserDefaults standardUserDefaults] setObject:_phoneTextField.text forKey:@"User_name"];
                 if ([[responseObject objectForKey:@"succ"] isEqualToString:@"succ"]) {
